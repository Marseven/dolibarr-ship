...
/**
 * \file    test/functional/CashControlFunctionalTest.php
 * \ingroup cashcontrol
...
/**
 * Class CashControlFunctionalTest
 *
 ...
 *
 * @package Testcashcontrol
 */
class CashControlFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modCashControl')->click();
		} else {
			// Disable the module
			$this->byHref('modCashControl')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/cashcontrol/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('cashcontrol/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/cashcontrol/admin/about.php');
		$this->authenticate();
		return $this->assertContains('cashcontrol/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('cashcontrolwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCashControl_CashControlTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCashControl_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
