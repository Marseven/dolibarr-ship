...
/**
 * \file        htdocs/modulebuilder/template/class/reservation.class.php
 * \ingroup     bookticket
...
class Reservation extends CommonObject
{
	 ...
	 */
	public $table_element = 'bookticket_reservation';
	 ...
	 */
	public $picto = 'reservation@bookticket';
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->bookticket->reservation->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->bookticket->reservation->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->bookticket->reservation->reservation_advance->validate))))
		 {
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$dirsource = $conf->bookticket->dir_output.'/reservation/'.$oldref;
				$dirdest = $conf->bookticket->dir_output.'/reservation/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->bookticket->dir_output.'/reservation/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/bookticket/reservation_card.php', 1).'?id='.$this->id;
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->BOOKTICKET_RESERVATION_ADDON)) {
			$conf->global->BOOKTICKET_RESERVATION_ADDON = 'mod_reservation_standard';
		}
		...
		if (!empty($conf->global->BOOKTICKET_RESERVATION_ADDON))
		{
			...
			$file = $conf->global->BOOKTICKET_RESERVATION_ADDON.".php";
			$classname = $conf->global->BOOKTICKET_RESERVATION_ADDON;
	...
	public function approve($user = null, $notrigger = 0)
	{
		...
		// Update request
		$sql = "UPDATE ".MAIN_DB_PREFIX."bookticket_reservation SET";
	...
	public function load_state_board()
    {
         ...
         $sql .= " FROM ".MAIN_DB_PREFIX."bookticket_reservation as t";
         $sql .= " WHERE t.status > 0";
         $sql .= " AND t.entity IN (".getEntity('reservation').")";

         $resql = $this->db->query($sql);
         if ($resql) {
             while ($obj = $this->db->fetch_object($resql)) {
                 $this->nb["reservation"] = $obj->nb;
             }
             $this->db->free($resql);
             return 1;
         } else {
