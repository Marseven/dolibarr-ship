 ...
 * Class for Passenger
 */
class Passenger extends CommonObject
{
	/**
	...
	public $fields = array(
		...
		'civility'        	=> array('type'=>'varchar(255)', 'label'=>'Civilité', 'enabled'=>1, 'visible'=>1, 'position'=>30, 'searchall'=>1, 'css'=>'minwidth300', 'cssview'=>'wordbreak', 'help'=>'Help text', 'showoncombobox'=>1),
		...
		'date_naissance'    => array('type'=>'date', 'label'=>'Date de naissance', 'enabled'=>1, 'visible'=>1, 'notnull'=>1, 'position'=>45, 'searchall'=>0, 'help'=>'Help text', 'css'=>'maxwidth75imp'),
		'accompagne'    	=> array('type'=>'varchar(10)', 'label'=>'Accompagne', 'enabled'=>1, 'visible'=>1, 'notnull'=> 1, 'default'=>1, 'index'=>1, 'position'=>20),
		...
		'fk_socid' 			=> array('type'=>'integer:Societe:societe/class/societe.class.php', 'label'=>'Tiers', 'picto'=>'user', 'enabled'=>1, 'visible'=>-2, 'notnull'=> 1, 'position'=>510, 'foreignkey'=>'societe.rowid'),
	...
	/**
	 * @var integer|string date_naissance
	 */
	public $date_naissance;

	/**
	 * @var string accompagne
	 */
	public $accompagne;
	...
	public function validate($user, $notrigger = 0)
	{
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'passenger/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'passenger/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("passenger").'</u>';
		...
		if (empty($notooltip))
		{
			...
			{
				$label = $langs->trans("Showpassenger");
		...
		$hookmanager->initHooks(array('passengerdao'));
	...
	public function getLinesArray()
	{
		...
		$objectline = new passengerLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_passenger = '.$this->id));
...
/**
 * Class passengerLine. You can also remove this and generate a CRUD class for lines objects.
 */
class passengerLine extends CommonObjectLine
{
	// To complete with content of an object passengerLine
	// We should have a field rowid, fk_passenger and position
