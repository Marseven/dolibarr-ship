...
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'agencelist';
...
// Security check
//$result = restrictedArea($user, 'agence', '', '', '', '', '', 0);

// List of fields to search into when doing a "search in all"
$fieldstosearchall = array(
	'c.label'=>"AgenceLabel",
	'c.labelshort'=>"AgenceLabelShort",
);
...
$arrayfields = array(
	'a.rowid'=>array('label'=>$langs->trans("ID"), 'checked'=>1),
	'a.label'=>array('label'=>$langs->trans("Label"), 'checked'=>1, 'position'=>10),
	'a.labelshort'=>array('label'=>$langs->trans("LabelShort"), 'checked'=>1, 'position'=>20),
	'a.ville'=>array('label'=>$langs->trans("Ville"), 'checked'=>1, 'position'=>30),
	'a.date_creation'=>array('label'=>$langs->trans("DateCreation"), 'checked'=>1, 'position'=>500),
	'a.tms'=>array('label'=>$langs->trans("DateModificationShort"), 'checked'=>1, 'position'=>500),
);
...
$title = $langs->trans("Agences");

$texte = $langs->trans("Agences");


$sql = 'SELECT DISTINCT a.rowid, a.label, a.labelshort, a.ville, a.entity,';
$sql .= ' a.date_creation, a.tms as date_update';

// Add fields from hooks
$parameters = array();
$sql .= ' FROM '.MAIN_DB_PREFIX.'bookticket_agence as a';
$sql .= ' WHERE a.entity IN ('.getEntity('agence').')';

if ($search_label)   $sql .= natural_search('a.label', $search_label);
if ($search_labelshort) $sql .= natural_search('a.labelshort', $search_labelshort);
...
if ($resql)
{
	...
	if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall)
	{
		...
		$id = $obj->rowid;
		header("Location: ".DOL_URL_ROOT.'/custom/bookticket/agence_card.php?id='.$id);
		exit;
	}
	...
	// Displays agence removal confirmation
	if (GETPOST('delagence')) {
		setEventMessages($langs->trans("AgenceDeleted", GETPOST('delagence')), null, 'mesgs');
	}
	...
	$newcardbutton .= dolGetButtonTitle($langs->trans('NewAgence'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/custom/bookticket/agence_card.php?action=create&type=0', '', $perm, $params);
	$label = 'NewAgence';
	...
	$modelmail = "agence";
	$objecttmp = new Agence($db);
	$trackid = 'agence'.$object->id;
	...
	if (!empty($arrayfields['a.labelshort']['checked']))
	{
		print '<td class="liste_titre left">';
	...
	if (!empty($arrayfields['a.labelshort']['checked'])) {
		print_liste_field_titre($arrayfields['a.labelshort']['label'], $_SERVER["PHP_SELF"], "a.labelshort", "", $param, "", $sortfield, $sortorder);
	}
	...
	if (!empty($arrayfields['a.date_creation']['checked'])) {
		print_liste_field_titre($arrayfields['a.date_creation']['label'], $_SERVER["PHP_SELF"], "a.date_creation", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	}
	if (!empty($arrayfields['a.tms']['checked'])) {
		print_liste_field_titre($arrayfields['a.tms']['label'], $_SERVER["PHP_SELF"], "a.tms", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	}
	print_liste_field_titre($selectedfields, $_SERVER["PHP_SELF"], "", '', '', '', $sortfield, $sortorder, 'center maxwidthsearch ');
	print "</tr>\n";


	$agence_static = new Agence($db);
	...
	while ($i < min($num, $limit))
	{
		...
		if (!empty($arrayfields['a.ville']['checked']))
		{
			print '<td class="tdoverflowmax200" title="'.dol_escape_htmltag($obj->ville).'">'.$obj->ville.'</td>';
			if (!$i) $totalarray['nbfield']++;
		}
