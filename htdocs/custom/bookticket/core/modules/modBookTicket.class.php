...
/**
 * 	\defgroup   bookticket     Module BookTicket
 *  \brief      BookTicket module descriptor.
 *
 *  \file       htdocs/bookticket/core/modules/modBookTicket.class.php
 *  \ingroup    bookticket
 *  \brief      Description and activation file for module BookTicket
 */
include_once DOL_DOCUMENT_ROOT.'/core/modules/DolibarrModules.class.php';

/**
 *  Description and activation class for module BookTicket
 */
class modBookTicket extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleBookTicketName' not found (BookTicket is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));

		// Module description, used if translation string 'ModuleBookTicketDesc' not found (BookTicket is name of module).
		$this->description = "BookTicketDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "BookTicketDescription";

		// Author
		$this->editor_name = 'Mebodo Aristide';
		$this->editor_url = 'https://www.mebodorichard.tech';
		...
		// Key used in llx_const table to save module status enabled/disabled (where BOOKTICKET is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
				 //'/bookticket/css/select2.min.css',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				 //'/bookticket/js/select2.min.js',
			),
		...
		// Example: this->dirs = array("/bookticket/temp","/bookticket/subdir");
		$this->dirs = array("/bookticket/temp");

		// Config pages. Put here list of php page, stored into bookticket/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@bookticket");
		...
		$this->langfiles = array("bookticket@bookticket");
		...
		//$this->automatic_activation = array('FR'=>'BookTicketWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('BOOKTICKET_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('BOOKTICKET_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		...
		if (!isset($conf->bookticket) || !isset($conf->bookticket->enabled)) {
			$conf->bookticket = new stdClass();
			$conf->bookticket->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@bookticket:$user->rights->bookticket->read:/bookticket/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		...
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->bookticket->enabled, $conf->bookticket->enabled, $conf->bookticket->enabled)
		);
		*/

		// Boxes/Widgets
		// Add here list of php file(s) stored in bookticket/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'bookticketwidget1.php@bookticket',
			//      'note' => 'Widget provided by BookTicket',
		...
		$this->cronjobs = array(
			...
			//      'label' => 'MyJob label',
			//      'jobtype' => 'method',
			//      'class' => '/bookticket/class/myobject.class.php',
			//      'objectname' => 'MyObject',
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->bookticket->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->bookticket->enabled', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->bookticket->enabled', 'priority'=>50)
		// );
		...
		//penalite
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Read penalite of BookTicket'; // Permission label
		$this->rights[$r][4] = 'penalite'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update penalite of BookTicket'; // Permission label
		$this->rights[$r][4] = 'penalite'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete penalite of BookTicket'; // Permission label
		$this->rights[$r][4] = 'penalite'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		...
		//agence
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Read agence of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update agence of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete agence of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		...
		//agence_user
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Read agence_user of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence_user'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update agence_user of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence_user'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete agence_user of BookTicket'; // Permission label
		$this->rights[$r][4] = 'agence_user'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		...
		//reservation
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Read reservation of BookTicket'; // Permission label
		$this->rights[$r][4] = 'reservation'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'read'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update reservation of BookTicket'; // Permission label
		$this->rights[$r][4] = 'reservation'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete reservation of BookTicket'; // Permission label
		$this->rights[$r][4] = 'reservation'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$this->rights[$r][5] = 'delete'; // In php code, permission will be checked by test if ($user->rights->bookticket->level1->level2)
		$r++;
		...
		$this->menu[$r++] = array(
			...
			'titre'=>'ModuleBookTicketName',
			'mainmenu'=>'bookticket',
			'leftmenu'=>'',
			'url'=>'/bookticket/bookticketindex.php',
		...
		$this->menu[$r++]=array(
			...
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
			'perms'=>'$user->rights->bookticket->penalite->read',			                // Use 'perms'=>'$user->rights->bookticket->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->menu[$r++]=array(
			...
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
			'perms'=>'$user->rights->bookticket->agence->read',			                // Use 'perms'=>'$user->rights->bookticket->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->menu[$r++]=array(
			...
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
			'perms'=>'$user->rights->bookticket->agence->write',			                // Use 'perms'=>'$user->rights->bookticket->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->menu[$r++]=array(
			...
			'mainmenu'=>'bookticket',
			'leftmenu'=>'classe',
			'url'=>'/bookticket/classeindex.php',
			'langs'=>'bookticket@bookticket',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
		...
		$this->menu[$r++]=array(
			...
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
			'perms'=>'$user->rights->bookticket->agence_user->read',			                // Use 'perms'=>'$user->rights->bookticket->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->menu[$r++]=array(
			...
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'ListAgenceUser',
			'mainmenu'=>'bookticket',
			'leftmenu'=>'bookticket_agence_user_list',
			'url'=>'/bookticket/agence_user_list.php',
			'langs'=>'bookticket@bookticket',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
		...
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=bookticket,fk_leftmenu=agence_user',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'ListUserCaisse',
			'mainmenu'=>'bookticket',
			'leftmenu'=>'bookticket_user_caisse_list',
			'url'=>'/bookticket/user_caisse_list.php',
			'langs'=>'bookticket@bookticket',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->bookticket->enabled',  // Define condition to show or hide menu entry. Use '$conf->bookticket->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			//'perms'=>'1', // Use 'perms'=>'$user->rights->bookticket->myobject->read' if you want your menu with a permission rules
			'perms'=>'$user->rights->bookticket->user_caisse->read',			                // Use 'perms'=>'$user->rights->bookticket->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		...
		$this->export_icon[$r]='myobject@bookticket';
		// Define $this->export_fields_array, $this->export_TypeFields_array and $this->export_entities_array
		$keyforclass = 'MyObject'; $keyforclassfile='/bookticket/class/myobject.class.php'; $keyforelement='myobject@bookticket';
		include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		//$this->export_fields_array[$r]['t.fieldtoadd']='FieldToAdd'; $this->export_TypeFields_array[$r]['t.fieldtoadd']='Text';
		//unset($this->export_fields_array[$r]['t.fieldtoremove']);
		//$keyforclass = 'MyObjectLine'; $keyforclassfile='/bookticket/class/myobject.class.php'; $keyforelement='myobjectline@bookticket'; $keyforalias='tl';
		//include DOL_DOCUMENT_ROOT.'/core/commonfieldsinexport.inc.php';
		$keyforselect='myobject'; $keyforaliasextra='extra'; $keyforelement='myobject@bookticket';
		include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
		//$keyforselect='myobjectline'; $keyforaliasextra='extraline'; $keyforelement='myobjectline@bookticket';
		//include DOL_DOCUMENT_ROOT.'/core/extrafieldsinexport.inc.php';
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/bookticket/sql/');
		if ($result < 0) return -1; // Do not activate module if error 'not allowed' returned when loading module SQL queries (the _load_table run sql with run_sql with the error allowed parameter set to 'default')

		// Create extrafields during init
		//include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
		//$extrafields = new ExtraFields($this->db);
		//$result1=$extrafields->addExtraField('bookticket_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'bookticket@bookticket', '$conf->bookticket->enabled');
		//$result2=$extrafields->addExtraField('bookticket_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'bookticket@bookticket', '$conf->bookticket->enabled');
		//$result3=$extrafields->addExtraField('bookticket_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'bookticket@bookticket', '$conf->bookticket->enabled');
		//$result4=$extrafields->addExtraField('bookticket_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'bookticket@bookticket', '$conf->bookticket->enabled');
		//$result5=$extrafields->addExtraField('bookticket_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'bookticket@bookticket', '$conf->bookticket->enabled');
		...
		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			...
			if ($myTmpObjectArray['includerefgeneration']) {
				$src = DOL_DOCUMENT_ROOT.'/install/doctemplates/bookticket/template_myobjects.odt';
				$dirodt = DOL_DATA_ROOT.'/doctemplates/bookticket';
