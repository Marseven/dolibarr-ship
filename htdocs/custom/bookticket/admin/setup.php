...
/**
 * \file    bookticket/admin/setup.php
 * \ingroup bookticket
 * \brief   BookTicket setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "bookticket@bookticket"));
...
$arrayofparameters = array(
	...
	'BOOKTICKET_MYPARAM2'=>array('css'=>'minwidth500', 'enabled'=>1)
);
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/bookticket/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
elseif ($action == 'setmod') {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'BOOKTICKET_'.strtoupper($tmpobjectkey)."_ADDON";
...
elseif ($action == 'set') {
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'BOOKTICKET_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$page_name = "BookTicketSetup";
llxHeader('', $langs->trans($page_name));

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($page_name), $linkback, 'object_bookticket@bookticket');

// Configuration header
$head = bookticketAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', '', -1, "bookticket@bookticket");
...
print load_fiche_titre($langs->trans("BticketsPDFModules"), '', '');
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								print '<td class="center">';
								$constforvar = 'BOOKTICKET_'.strtoupper($myTmpObjectKey).'_ADDON';
