...
/**
 *	\file       bookticket/agenceindex.php
 *	\ingroup    bookticket
 *	\brief      Home page of agence left menu
 */
...
$object = new Agence($db);

if ($id > 0)
{
	$result = $object->fetch($id);

	if (!empty($conf->agence->enabled)) $upload_dir = $conf->agence->multidir_output[$object->entity].'/'.get_exdir(0, 0, 0, 0, $object, 'agence');
	elseif (!empty($conf->service->enabled)) $upload_dir = $conf->service->multidir_output[$object->entity].'/'.get_exdir(0, 0, 0, 0, $object, 'agence');

	if (!empty($conf->global->AGENCE_USE_OLD_PATH_FOR_PHOTO))    // For backward compatiblity, we scan also old dirs
	{
		if (!empty($conf->agence->enabled)) $upload_dirold = $conf->agence->multidir_output[$object->entity].'/'.substr(substr("000".$object->id, -2), 1, 1).'/'.substr(substr("000".$object->id, -2), 0, 1).'/'.$object->id."/photos";
...
if (!empty($canvas))
{
	...
	$objcanvas->getCanvas('agence', 'card', $canvas);
}

// Security check
//$result = restrictedArea($user, 'produit|service', $fieldvalue, 'agence&agence', '', '', $fieldtype);
...
$usercanread = $user->rights->bookticket->agence->read;
$usercancreate = $user->rights->bookticket->agence->write;
$usercandelete = $user->rights->bookticket->agence->delete;
...
if (empty($reshook))
{
	...
	$upload_dir = $conf->agence->dir_output;
		...
		if (GETPOST('cancel', 'alpha'))
		{
			...
			if ($object->id > 0)
			{
				...
				if (!$error && $object->check())
				{
					...
					if (count($object->errors)) setEventMessages($object->error, $object->errors, 'errors');
					else setEventMessages($langs->trans("ErrorAgenceBadRefOrLabel"), null, 'errors');
					$action = 'edit';
				}
			}
		}
			...
			if ($object->id > 0)
			{
				...
				if ($object->check())
				{
					...
					if ($id > 0)
					{
						...
						if ($object->error == 'ErrorAgenceAlreadyExists')
						{
							$db->rollback();
	...
	if ($action == 'delete' && $usercandelete)
	{
		...
		if ($result > 0)
		{
			header('Location: '.DOL_URL_ROOT.'/custom/bookticket/agence_list.php?delagence='.urlencode($object->rowid));
			exit;
		} else {
			setEventMessages($langs->trans($object->error), null, 'errors');
			$reload = 0;
...
$title = $langs->trans('AgenceCard');
$helpurl = '';
$shortlabel = dol_trunc($object->label, 16);
$title = $langs->trans('agence')." ".$shortlabel." - ".$langs->trans('Card');
$helpurl = 'EN:Module_Agences|FR:Module_Produits|ES:M&oacute;dulo_Agenceos';
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create' && $usercancreate) {
		...
		if ($action == 'edit' && $usercancreate)
		{
			...
			$head = agence_prepare_head($object);
			$titre = $langs->trans("CardAgence".$object->type);
			$picto =  'agence';
			...
			// Fiche en mode visu
			$head = agence_prepare_head($object);
			$titre = $langs->trans("CardAgence");
			$picto = 'agence';

			print dol_get_fiche_head($head, 'card', $titre, -1, $picto);

			$linkback = '<a href="'.DOL_URL_ROOT.'/custom/bookticket/agence_list.php?restore_lastsearch_values=1&type=">'.$langs->trans("BackToList").'</a>';

			$shownav = 1;
			if ($user->socid && !in_array('agence', explode(',', $conf->global->MAIN_MODULES_FOR_EXTERNAL))) $shownav = 0;
...
// Confirm delete agence
if (($action == 'delete' && (empty($conf->use_javascript_ajax) || !empty($conf->dol_use_jmobile)))	// Output when action = clone if jmobile or no js
	|| (!empty($conf->use_javascript_ajax) && empty($conf->dol_use_jmobile)))							// Always output when not jmobile nor js
{
	$formconfirm = $form->formconfirm("card.php?id=".$object->id, $langs->trans("DeleteAgence"), $langs->trans("ConfirmDeleteAgence"), "confirm_delete", '', 0, "action-delete");
}
	...
	|| (!empty($conf->use_javascript_ajax) && empty($conf->dol_use_jmobile)))							// Always output when not jmobile nor js
{
	...
	$formquestionclone = array(
		...
		array('type' => 'checkbox', 'name' => 'clone_content', 'label' => $langs->trans("CloneContentAgence"), 'value' => 1),
		array('type' => 'checkbox', 'name' => 'clone_categories', 'label' => $langs->trans("CloneCategoriesAgence"), 'value' => 1),
	);


	$formconfirm .= $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneAgence', $object->rowid), 'confirm_clone', $formquestionclone, 'yes', 'action-clone', 350, 600);
}
;
// Print form confirm
print $formconfirm;*/
...
if ($action != 'create' && $action != 'edit')
{
	...
	$parameters = array();
		...
		if ($usercancreate && $object->status == Agence::STATUS_DRAFT)		// If draft
		{
		...
		if ($usercandelete)
		{
			if (!isset($object->no_button_delete) || $object->no_button_delete <> 1)
			{
				if (!empty($conf->use_javascript_ajax) && empty($conf->dol_use_jmobile))
				{
					...
					print '<a class="butActionDelete" onclick="return confirm(\'Voulez-vous vraiment supprimer cette agence ! \');" href="'.$_SERVER["PHP_SELF"].'?action=delete&amp;token='.newToken().'&amp;id='.$object->id.'">'.$langs->trans("Delete").'</a>';
				}
			} else {
				print '<a class="butActionRefused classfortooltip" href="#" title="'.$langs->trans("AgenceIsUsed").'">'.$langs->trans("Delete").'</a>';
			}
		} else {
